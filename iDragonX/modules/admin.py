...
from iDragonX.helpers.pyrohelper import get_arg, get_args
...
『 **• Admin Tools** 』
  ...
  `{PREFIX}kick` -> Kicks the user out of the group.
  `{PREFIX}pin` -> pins a message.
  `{PREFIX}del` -> delete a message.
  `{PREFIX}purge` -> purge message(s)
  `{PREFIX}invite` -> add user to chat.
"""
    }
)
