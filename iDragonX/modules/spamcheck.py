...
CMD_HELP.update(
    {
        "SpamCheck": f"""
『 **• SpamCheck** 』
  `{PREFIX}spamcheck` -> Check Spam Status of a User in Different Anti-Spam Systems
  `{PREFIX}anon` -> AnonymousArmy Ban check
  `{PREFIX}deai` -> DEAI Ban Check
  `{PREFIX}spb` -> SPB Ban Check
  `{PREFIX}cas` -> CAS Ban Check
  `{PREFIX}rose` -> RoseSupport Ban Check
  `{PREFIX}sw` -> SpamWatch Ban Check
  `{PREFIX}sibyl` -> Sibyl Gban Check
"""
    }
)
...
@app.on_message(filters.me & filters.command(['einfo', 'spamcheck', 'sw', 'spamwatch', 'aa', 'anon', 'deai', 'spb', 'spamprotection', 'cas', 'combot', 'rose', 'sibyl'], PREFIX))
